# Import statements
import json  # Import the built-in json package
from employee import details, employee_name, age, title  # Import required function and variables
...
def create_dict(name, age, title):
    ...
    Returns:
        dict - A dictionary that maps "first_name", "age", and "title" to the
               ...
               title - string)
    """
    # Create the dictionary with typecasting
    employee_dict = {
        "first_name": str(name),  # Ensure name is a string
        "age": int(age),         # Ensure age is an integer
        "title": str(title)      # Ensure title is a string
    }
    return employee_dict  # Return the created dictionary
...
def write_json_to_file(json_obj, output_file):
    ...
    """
    # Open the file in write mode and write the JSON object to it
    with open(output_file, "w") as newfile:
        newfile.write(json_obj)  # Write the JSON object to the file
...
def main():
    ...
    # Convert the employee_dict to a JSON string using json.dumps
